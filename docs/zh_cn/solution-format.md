...
应使用重新排列后的 `attacker_index`。数量确定时可填写 `ally_count`；随机效果会
改变数量时可省略，由执行器在动作前识别。
...
`attack` 的 `ally_count` 与所有随从目标的 `target.count` 是可选字段。填写时直接
使用对应数量的固定布局；省略时，执行器会在每次动作开始前重新截图，通过随从
左下角的蓝色攻击力数字识别该侧实时随从数量，再计算当前卡位。识别结果不会在
后续动作间复用，因此随机效果造成随从离场或数量变化时，应省略这些计数。

自动模式只统计随从，不能识别占用场上卡位的护符。若所计算的一侧仍有护符，
必须显式填写包含随从、护符等所有占位对象的总数。`activate_amulet.ally_count`
始终必填，因为启动护符本身就说明我方场上存在非随从对象。
...
`ally_follower`。随从目标必须填写目标序号 `index`；`count` 可显式填写当前场上
对象总数，也可在只有随从占位时省略并实时识别。显式总数包含会占用并改变卡位
布局的护符等对象，而不只是可攻击的随从。

### 随从攻击

数量确定，或场上混有护符时，可以显式填写计数：
...
随机效果可能改变双方随从数量时，可以同时省略 `ally_count` 和
`target.count`。下面的动作会在执行前分别识别双方随从数量，再由当前我方
第一个随从攻击当前敌方第一个随从：
...
识别对应按钮，因此不需要为不同卡牌填写按钮坐标。普通进化使用左侧的“进化”
按钮：
...
`target.count` 遵循随从目标的通用规则：只有随从时可以省略并实时识别，混有护符
时必须填写该动作执行前我方场上所有占用卡位的对象总数。进化动画较长时可增加
`after_ms`，避免下一步在场面重新排列完成前执行。
可用 `detail_delay_ms` 调整点击随从后等待详情框出现的时间，使用
`evolution_timeout_ms` 调整等待进化按钮识别成功的最长时间。

### 启动护符

`activate_amulet` 会根据动作执行时我方场上所有占用卡位的对象数量，点击指定
护符，再从动态高度的卡牌详情框中 OCR 查找“启动”按钮。若护符效果需要指定
对象，可同时填写 `target`：
...
`amulet_index` 是护符在当前全部我方场上对象中的从左起序号；`ally_count` 是
动作执行前我方场上对象总数，两者均必填。若 `target` 指向随从，其
`target.count` 遵循通用可选规则：目标侧只有随从时可省略，目标侧混有护符时
必须显式填写全部占位对象数。`detail_delay_ms`、`target_delay_ms` 和
`activation_timeout_ms` 分别控制详情框等待、指定对象等待和启动按钮识别超时。

### 开局换牌与能量点

人工固定解法中的开局换牌预留 `mulligan` 动作，再点击“决定”。该解法动作尚未接入
逐关参数，因此当前不能使用；自动战斗的通用换牌能力由 Battle Profile 的 `mulligan`
配置提供，详见[用户对战策略 JSON](./battle-profile.md)。
